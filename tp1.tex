...
\tadOperacion{posicionesContiguas}{posicion}{conj(posicion)}{}
...
\tadAxioma{actualizarAs(pos,dic,c)}{
	...
	ELSE {
		\IF hayHippiesAtrapados(pos,c) $\land$ $\neg$ hayEstudiantesAtrapados(pos,c) THEN premiarAs(pos,claves(dic),c)
		ELSE{
...
\tadAxioma{hayHippiesAtrapados(pos,c)}{
	\IF $\emptyset$?(posConHippies(posValidasCont(posicionesContiguas(pos),grilla(c)),Hippies(c))) THEN false
	ELSE revisarHippies(posConHippies(posValidasCont(posicionesContiguas(pos),grilla(c)),Hippies(c)),Hippies(c)),c)
	FI
}
...
\tadAxioma{posicionesContiguas(pos)}{
	Ag(($\pi_1$(pos)+1,$\pi_2$(pos)), Ag(($\pi_1$(pos)-1,$\pi_2$(pos)), Ag(($\pi_1$(pos),$\pi_2$(pos)+1), Ag(($\pi_1$(pos),$\pi_2$(pos)-1), $\emptyset$)
}
...
\tadAxioma{revisarHippies(conjpos,c)}{
	...
	ELSE{
		\IF esAtrapadoH?(DameUno(conjpos), posValidasCont(posicionesContiguas(DameUno(conjpos)),grilla(c)),c)
		THEN true
...
\tadAxioma{hayEstudiantesAtrapados(pos,c)}{
	\IF $\emptyset$?(posConEstudiantes(posValidasCont(PosicionesContiguas(pos), grilla(c)), Estudiantes(c))) THEN false
	ELSE revisarEstudiantes(posConEstudiantes(posValidasCont(PosicionesContiguas(pos), grilla(c)), Estudiantes(c)),c)
	FI
}
...
\tadAxioma{revisarEstudiantes(conjpos,c)}{
	\IF $\emptyset$?(conjpos) THEN false
	ELSE{
		\IF esAtrapadoE?(DameUno(conjpos), posValidasCont(posicionesContiguas(DameUno(conjpos),grilla(c),c))) THEN true
...
\tadAxioma{esAtrapadoE?(posE,concont,c)}{
	\IF hayPosVacia?(conjcont,c) THEN false
	ELSE {
		\IF DameUno(conjcont) $\in$ conjuntoPosAs(claves(asshole(c),asshole(c)) THEN true
...
% Me molestaba que aparezca y lo comenté, revisen si sirve esta funcion!!!!
%\tadAxioma{posVacia?(c, pos)}{posSinObstaculo?(grilla(c), pos) $\land$ }
