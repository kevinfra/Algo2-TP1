...
\tadOperacion{crearCampus}{grilla/g, dicc(placa, as)/ass}{campus}{posValidas?(g, ass) $\land$ posVacias?(g, ass) $\land$ sinCapturas?(ass) $\land$ sinSanciones?(ass)}
\tadOperacion{entraEstudiante}{campus/c, posicion/p}{campus}{posValida?(c, p) $\land$ posVacia?(c, p) $\land$ esEntrada?(c, p)}
\tadOperacion{entraHippie}{campus/c, posicion/p}{campus}{posValida?(c, p) $\land$ posVacia?(c, p) $\land$ esEntrada?(c, p)}
...
\tadOperacion{posVacias?}{grilla, dicc(placa, as)}{bool}{}
...
\tadAlinearAxiomas{false $\oluego$ $x$ xzx xxzx zzzzzx z }
