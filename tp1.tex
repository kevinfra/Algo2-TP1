...
\tadAxioma{hayHippies?(c)}{$\#$Hippies(c) $> 0$}
...
\tadAxioma{hAtrapados(conjContiguo,c)}{ \IF $\emptyset$?(conjContiguo) THEN $\emptyset$
	ELSE{
		...
		THEN{
			\IF esAtrapadoH?(posContiguas(dameUno(conjContiguo),c))
...
\tadAxioma{premiarYSancionar(conjP,c)}{ \IF $\emptyset$?(conjP) THEN asshole(c)
	ELSE{
		\IF $\pi_2$(dameUno(conjP))$=$false
		THEN definir($\pi_1$(dameUno(conjP)),\\ (Obtener($\pi_1$dameUno(conjP),asshole(c).posicion,\\ Obtener($\pi_1$dameUno(conjP),asshole(c).capturas +1,\\ Obtener($\pi_1$dameUno(conjP),asshole(c)).sanciones),\\  premiarYSancionar(sinUno(conjP),c))
