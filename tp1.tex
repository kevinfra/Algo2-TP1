...
\tadAxioma{seConvierteEnHippie?(c, pos)}{hayHippiesCerca(posContiguas(pos, grilla(c)), c)}
\tadAxioma{hayHippiesCerca(posiciones, c)}{\IF $\emptyset$?(posiciones) 
	THEN false
	ELSE {
	\- \IF esHippie?(c, DameUno(posiciones)) 
		THEN hayOtrosHippies(SinUno(posciciones), c)
		ELSE hayHippiesCerca(posiciones, c)FI}
	FI
}
\tadAxioma{hayOtrosHippies(posiciones, c)}{\IF $\emptyset$?(posiciones)
	THEN false
	ELSE { \- \IF esHippie?(c, DameUno(posiciones))
			THEN true
			ELSE hayOtrosHippie(SinUno(posiciones), c) FI }
	FI
}

\tadAxioma{nuevaPosHippie(c,pos)}{\IF hayEstudiantes(c) THEN
	perseguirEstudiantesCercanos(c, pos) ELSE
	pos
	FI
}
\tadAxioma{hayEstudiantes(c)}{$\#$estudiantes(c) $> 0$}

\tadAxioma{perseguirEstudiantesCercanos(c,pos)}{NuevaPosicion(pos, estudiantesCercanos(estudiantes(c), pos), c)}

\tadAxioma{estudiantesCercanos(estudiantes, posActual)}{ \IF esCercano(DameUno(estudiantes), posActual, estudiantes) 
	THEN Ag(DameUno(estudiantes), estudiantesCercanos(SinUno(estudiantes), posActual))
	ELSE estudiantesCercanos(SinUno(estudiantes), posActual)
	FI
}
\tadAxioma{esCercano(objetivo, posActual, conjObjetivos)}{(distancia(objetivo.x, posActual.x) \newline $+$ distancia(objetivo.y, posActual.y)) \newline $=$ elMasCercano(posActual, conjObjetivos)}

\tadAxioma{elMasCercano(posActual, conjObjetivos)}{
	\IF $\#$conjObjetivos $=$ 1 THEN DameUno(conjObjetivos) ELSE min( (distancia(DameUno(conjObjetivos).x, posActual.x) $+$ distancia(DameUno(conjObjetivos).y, posActual.y)), \newline elMasCercano(posActual, SinUno(conjObjetivos)) ) FI} 
