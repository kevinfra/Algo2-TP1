...
\tadOperacion{posContiguas}{posicion}{conj(posicion)}{}
...
\tadAxioma{asshole(movAS(c, p))}{ \IF $\pi_2$(Obtener(p,asshole(c))) $\geq3$ 
    THEN asshole(c)
    ELSE {
...
\tadAxioma{hayHippiesAtrapados(pos,c)}{
	\IF $\emptyset$?(posConHippies(posValidasCont(posContiguas(pos),grilla(c)),Hippies(c))) THEN false
	ELSE revisarHippies(posConHippies(posValidasCont(posContiguas(pos),grilla(c)),Hippies(c)),Hippies(c)),c)
	FI
}
...
\tadAxioma{revisarHippies(conjpos,c)}{
	...
	ELSE{
		\IF esAtrapadoH?(DameUno(conjpos), posValidasCont(posContiguas(DameUno(conjpos)),grilla(c)),c)
		THEN true
...
\tadAxioma{hayEstudiantesAtrapados(pos,c)}{
	\IF $\emptyset$?(posConEstudiantes(posValidasCont(posContiguas(pos), grilla(c)), Estudiantes(c))) THEN false
	ELSE revisarEstudiantes(posConEstudiantes(posValidasCont(posContiguas(pos), grilla(c)), Estudiantes(c)),c)
	FI
}
...
\tadAxioma{revisarEstudiantes(conjpos,c)}{
	\IF $\emptyset$?(conjpos) THEN false
	ELSE{
		\IF esAtrapadoE?(DameUno(conjpos), posValidasCont(posContiguas(DameUno(conjpos),grilla(c),c))) THEN true
...
\tadAxioma{hAtrapados(conjH,c)}{
	...
	ELSE{
		\IF esAtrapadoH(DameUno(conjH), posValidasCont(posContiguas(DameUno(conjH)),grilla(c)), c)
