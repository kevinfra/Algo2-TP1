...
\tadAxioma{hippies(entraEstudiante(c, pos))}{hippies(c) - hippiesAdiestrados(c, pos) - hippiesCapturados(c, pos)}
...
\tadAxioma{posQueAcercanAUno(c, pos, objetivo)}{
    iterarDirecciones(objetivo, posContiguas(c, pos), \\distanciaTaxista(pos, objetivo))
}

\tadAxioma{iterarDirs(objetivo, contiguas, dist)}{
    \IF $\emptyset$?(contiguas) THEN $\emptyset$
    ELSE {
        \IF distanciaTaxista(objetivo, dameUno(contiguas)) < dist THEN Ag(dameUno(contiguas), \\iterarDirecciones(objetivo, sinUno(contiguas), dist))
        ELSE iterarDirecciones(objetivo, sinUno(contiguas), dist)
        FI
    } 
    FI
}
...
\tadAxioma{asMasVigilante(c)}{elDeMenorPlaca(losMasVigilantes(asshole(c), numCapturasMax(conjuntoDeCapturas(asshole(c)))))}
...
\tadAxioma{numCapturasMax(conjCapturas)}{\IF $\#$conjCapturas $= 1$ THEN dameUno(conjCapturas)
ELSE max(dameUno(conjCapturas), numCapturasMax(sinUno(conjCapturas))) FI }
