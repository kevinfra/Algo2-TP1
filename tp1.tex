...
\tadAxioma{esHippie?(c, p)}{ \IF p $\in$ hippies(c)
    THEN true
    ELSE {
        false
    }
...
\tadAxioma{esEstudiante?(c, p)}{ \IF p $\in$ estudiantes(c)
    THEN true
    ELSE {
        false
    }
...
\tadAxioma{esEntrada?(c,p)}{ \IF $\pi_2$(p) = 0 $\land$ $\pi_2$(p) = alto(grilla(c)) - 1 
    THEN true
    ELSE {
       false
    }
...
\tadAxioma{esAS?(c, p)}{ \IF def?(p,asshole(c))
    THEN true
    ELSE {
        false
    }
