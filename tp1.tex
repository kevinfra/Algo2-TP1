% TODO agregar esAtrapadoH?
% TODO agregar posContiguas
% TODO parametros nuevaPosicion
% TODO estaVaciaPos? -> posVacia?
% TODO agregar distTaxi
...
\tadOperacion{losMasVigilantes}{dicc(placa,as)/ass, nat/NDeCapt}{conj(placa)}{nDeCaptValido(NDeCapt,ass)} %hay que hacer la restriccion
...
\tadAxioma{posSinAS?(c, pos)}{ pos $\notin$ conjuntoPosAs(claves(asshole(c),asshole(c)))}
...
\tadAxioma{esAS?(c, p)}{ def?(p,asshole(c))}
...
\tadAxioma{cantHippiesAdyacentes(c, pos)}{contarHippies(c, posContiguas(c, pos))}
...
\tadAxioma{estudiantesFalopeados(c, pos)}{buscarFalopeados(c, posContiguas(c, pos))}
...
\tadAxioma{posQueAcercanAUno(c, pos, objetivo)}{
    iterarDirs(objetivo, posContiguas(c, pos), distTaxi(pos, objetivo))
}
...
\tadAxioma{hippiesConvertidos(c, pos)}{buscarConvertidos(c, posContiguas(c, pos))}
...
\tadAxioma{cantEstuAdyacentes(c, pos)}{contarEstudiantes(c, posContiguas(c, pos))}
...
\tadAxioma{hippiesCapturados(c, pos)}{buscarCapturados(c, posContiguas(c, pos))}
\tadAxioma{buscarCapturados(c, contiguas)}{
    \IF $\emptyset$?(contiguas) THEN $\emptyset$
    ELSE {
        \IF esAtrapadoH?(c, posContiguas(c, dameUno(contiguas))) \\THEN Ag(dameUno(contiguas), \\buscarCapturados(c, sinUno(contiguas)))
...
\tadAxioma{asMasVigilante(c)}{elDeMenorPlaca(losMasVigilantes(asshole(c), \\numCapturasMax(conjuntoDeCapturas(asshole(c)))))}
\tadAxioma{elDeMenorPlaca(placas)}{\IF $\#$placas $= 1$
	THEN dameUno(placas)
	ELSE min(dameUno(placas), elDeMenorPlaca(sinUno(placas)))
	FI
}
\tadAlinearAxiomas{losMasVigilantes(asshole, numCapturas)}
\tadAxioma{losMasVigilantes(asshole, numCapturas)}{ \IF $\#$claves(asshole) $= 1$
    THEN dameUno(claves(asshole))
    ELSE{
        \IF capturas(dameUno(claves(asshole))) $=$ numCapturas THEN Ag(dameUno(claves(asshole)), \\losMasVigilantes(borrar(dameUno(claves(asshole)), \\asshole), numCapturas))
        ELSE losMasVigilantes(borrar(dameUno(claves(asshole)), \\asshole), numCapturas)
        FI
    }
    FI
...
\tadAlinearAxiomas{conjuntoDeCapturas(asshole)}
...
\tadAxioma{conjuntoDeCapturas(asshole)}{
    \IF $\#$ claves(asshole) $= 1$ THEN obtener(dameUno(claves(asshole)),asshole).capturas 
    ELSE Ag(obtener(dameUno(claves(asshole))).capturas, \\conjuntoDeCapturas(borrar(dameUno(claves(asshole))), asshole)) 
    FI
}
